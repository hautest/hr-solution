...
import type { AttendanceRecord, StatusChange, EditHistory } from '@/types/employee'
...
export function EditAttendanceDialog({ isOpen, onClose, record, onSave }: EditAttendanceDialogProps) {
  ...
  const addEditHistory = (action: 'add' | 'edit' | 'delete', description: string, oldValue?: any, newValue?: any) => {
  ...
  const addStatusChange = (newStatusChangeData: Omit<StatusChange, 'id'>) => {
    ...
    setEditingRecord(prev => ({
      ...prev,
      statusChanges: [...prev.statusChanges, newChange].sort((a, b) => a.time.localeCompare(b.time))
    }))
    
    addEditHistory('add', `새 상태 변경 추가: ${newChange.time} ${getStatusText(newChange.status)}`)
  }

  const updateStatusChange = (id: string, field: keyof StatusChange, value: any) => {
    setEditingRecord(prev => {
  ...
  const deleteStatusChange = (id: string) => {
    ...
    if (changeToDelete) {
      ...
      addEditHistory('delete', `상태 변경 삭제: ${changeToDelete.time} ${getStatusText(changeToDelete.status)}`)
    }
  }

  const saveChanges = () => {
    // 총 근무시간과 휴식시간 재계산 (간단한 예시)
    const totalHours = editingRecord.statusChanges.filter(c => 
      c.status === 'work_office' || c.status === 'work_remote'
    ).length * 1 // 실제로는 더 복잡한 계산 필요
    
    const updatedRecord = {
      ...editingRecord,
      totalWorkHours: totalHours,
      totalBreakHours: editingRecord.statusChanges.filter(c => c.status === 'break').length * 0.5
    }
              ...
              {editingRecord.statusChanges.map((change) => (
                <div key={change.id} className="flex items-center gap-3 p-3 border rounded-lg">
                  <Input
                    type="time"
                    value={change.time}
                    onChange={(e) => updateStatusChange(change.id, 'time', e.target.value)}
                    className="w-32"
                  />
                  
                  <Select
                    value={change.status}
                    onValueChange={(value) => updateStatusChange(change.id, 'status', value)}
                  >
                  ...
                  <Input
                    ...
                    onChange={(e) => updateStatusChange(change.id, 'slackKeyword', e.target.value)}
                    className="flex-1"
                  />
                  ...
                  <Input
                    ...
                    onChange={(e) => updateStatusChange(change.id, 'slackMessageLink', e.target.value)}
                    className="flex-1"
                  />
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {editingRecord.editHistory.map((history) => (
                <div key={history.id} className="text-sm p-2 bg-gray-50 rounded">
                  ...
                  <div className="text-muted-foreground">수정자: {history.editedBy}</div>
                </div>
              ))}
            </div>
          </div>
